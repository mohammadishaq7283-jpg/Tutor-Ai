...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        /* MAIN CONTENT */
        .main-content { flex: 1; display: flex; flex-direction: column; width: 100%; height: 100%; position: relative; transition: 0.3s; }
        
        .header { padding: 10px 15px; display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid var(--border-color); background: var(--bg-color); }
        ...
        .auth-box { background: var(--card-bg); padding: 25px; border-radius: 12px; max-width: 400px; margin: 20px auto; box-shadow: 0 4px 10px rgba(0,0,0,0.1); text-align: center; width: 100%; }
        ...
        .message { padding: 10px 15px; border-radius: 15px; max-width: 85%; line-height: 1.5; font-size: 15px; position: relative; }
        ...
        .chat-input-area { padding: 10px; border-top: 1px solid var(--border-color); display: flex; gap: 10px; background: var(--bg-color); align-items: center; }
        .input-wrapper { flex: 1; position: relative; display: flex; align-items: center; }
        .chat-input-area input { margin: 0; padding-right: 40px; }
        
        .camera-btn { position: absolute; right: 10px; background: none; border: none; color: #888; font-size: 18px; cursor: pointer; }
        .send-btn { width: 45px; height: 45px; background: #007bff; color: white; border: none; border-radius: 50%; cursor: pointer; display: flex; align-items: center; justify-content: center; }
...
<div class="main-content">
    ...
    <div id="screen-chat" class="screen" style="padding: 0; display: none; flex-direction: column;">
        <div style="padding: 10px; background: var(--card-bg); border-bottom: 1px solid var(--border-color); display: flex; align-items: center; justify-content: space-between;">
...
<script>
    ...
    function startChat(subject) {
        ...
        if(isGuest) greeting = "Hello Guest!";

        document.getElementById('chat-history').innerHTML = `
            <div class="message ai-msg">
                ${greeting} I am your <b>${subject}</b> Tutor.
            </div>`;
    ...
    function updateHistoryUI() {
        ...
        list.innerHTML = chatSessions.map(s => `
            <div class="history-item" onclick="startChat('${s.subject}'); toggleSidebar();">
                <i class="far fa-comments"></i> ${s.subject}
            </div>
        `).join('');
    }

    function triggerCamera() { document.getElementById('camera-input').click(); }
    function handleImageSelect() { alert("Image selected! (Feature coming soon)"); }
